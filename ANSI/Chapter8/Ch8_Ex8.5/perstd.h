...
typedef struct _iobuf{
    ...
    int flag; // mode of file access: 文件状态标志
...
// 利用枚举类型定义flag字段可能的状态,采用二进制掩码的方式来表示
enum _flags{
...
int fclose(FILE *fp); // 声明fclose函数,用于关闭文件
int fflush(FILE *fp); // 声明fflush函数,用于刷新文件缓冲区
int fseek(FILE *fp, long offset, int origin); // 声明fseek函数,用于设置文件位置指针

/*这里我们在宏定义里面使用了()来括起了操作数,这是因为我们需要利用括号的最高优先级来保证我们的操作顺序的正确性 */
#define feof(p) (((p)->flag&_EOF)!=0)// 判断是否到达文件末尾
#define ferror(p) (((p)->flag&_ERR)!=0)// 判断是否发生错误
...
否则,我们就调用_flushbuf函数来刷新缓冲区,并写入字符.
*/
